...
var MessagesZhCN = map[string]string{
	...
	"validation.invalid_error_code_filter":   "无效的错误码过滤器，必须是大于等于 0 的整数",
	...
	"config.rate_limit_cooldown":             "限流冷却时间（秒）",
	"config.rate_limit_cooldown_desc":        "密钥触发 429 限流后的冷却等待时间（秒），冷却期满自动恢复为可用状态。",
